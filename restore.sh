...
exit_message() {
    ...
    if [[ -n $version_current && -n $git_hash ]]; then
        print "* lik-mac-compat $version_current ($git_hash)"
    else
        print "* lik-mac-compat"
    fi
    if [[ -n $platform ]]; then
...
display_help() {
    echo ' *** lik-mac-compat ***
  - Script by The Tech Peak on YouTube -
...
set_tool_paths() {
    ...
    if [[ $OSTYPE == "linux"* ]]; then
           ...
           [[ $(hostname 2>/dev/null) == "mint" && $USER == "mint" ]]; then
            ...
            if [[ $(pwd) == "/home/"* ]]; then
                ...
                if [[ $(lsblk -o label | grep -c "casper-rw") == 1 || $(lsblk -o label | grep -c "persistence") == 1 ]]; then
                    log "Detected lik-mac-compat running on persistent storage."
                    live_session_str+=" - Persistent storage"
                else
                    warn "Detected lik-mac-compat running on temporary storage."
                    print "* You may run out of space and get errors during the restore process."
                    print "* Please move lik-mac-compat to a drive that is NOT used for the live USB."
                    print "* This may mean using another external HDD/flash drive to store lik-mac-compat on."
        ...
        if [[ $(pwd) == *"/media/"* ]]; then
            ...
            print "* If this is the case, try moving lik-mac-compat to the Desktop or Documents folder."
        ...
        if [[ $device_argmode != "none" && $device_disable_usbmuxd != 1 && ! -e ../resources/new ]]; then
            ...
            if [[ $othertmp == 0 ]]; then
                ...
                warn "Detected existing tmp folder(s), there might be other lik-mac-compat instance(s) running"
        ...
        error "Running lik-mac-compat on iOS is not supported (yet)" \
        ...
        if [[ $mac_majver == 10 ]]; then
            ...
            case $mac_minver in
                7 ) mac_name="Lion";;
                8 ) mac_name="Mountain Lion";;
                9 ) mac_name="Mavericks";;
                10 ) mac_name="Yosemite";;
    ...
    if [[ ! -d $dir ]]; then
        error "Failed to find bin directory ($dir), cannot continue." \
              "* Re-download lik-mac-compat from releases (or do a git clone/reset)"
    fi
    if [[ $device_sudoloop == 1 ]]; then
        $sudo chmod +x $dir/*
        if [[ $? != 0 ]]; then
            error "Failed to set up execute permissions of binaries, cannot continue. Try to move lik-mac-compat somewhere else."
        fi
    else
        chmod +x $dir/*
    fi
...
install_depends() {
    ...
    if [[ $platform == "linux" ]]; then
        print "* lik-mac-compat will be installing dependencies from your distribution's package manager"
        print "* Enter your user password when prompted"
        print "* Your password input may not be visible, but it is still being entered."
...
version_get() {
    ...
    if [[ -d .git ]]; then
        ...
        if [[ $no_version_check != 1 ]]; then
            warn "Your copy of lik-mac-compat is downloaded incorrectly. Do not use the \"Code\" button in GitHub."
            print "* Please download lik-mac-compat using git clone or from GitHub releases: https://github.com/LukeZGD/Legacy-iOS-Kit/releases"
        fi
    fi
...
version_check() {
    ...
    if [[ -z $version_latest ]]; then
        ...
        if [[ -z $version_current ]]; then
            ...
            print "* A newer version of lik-mac-compat is available."
...
device_get_info() {
    ...
    if [[ $device_argmode == "none" ]]; then
        ...
        if [[ $platform == "linux" ]]; then
            ...
            if [[ $othertmp != 0 ]]; then
                print "* If it fails to detect devices, try to delete all \"tmp\" folders in your lik-mac-compat folder"
            fi
        fi
    ...
    if (( device_proc > 10 )); then
        ...
        print "* ECID: $device_ecid"
        echo
        warn "This device is mostly not supported by lik-mac-compat."
    ...
    case $mode in
        ...
        "jailbreak" )
            ...
            if [[ $device_type == "iPhone2,1" && $vers == "3."* ]]; then
                ...
                # 3.1.3â€“4.1 untether must be extracted before data partition mount
                case $vers in
                    4.[10]* | 3.2* | 3.1.3 )
...
menu_print_info() {
    ...
    print " *** lik-mac-compat ***"
    print " - Script by The Tech Peak on YouTube -"
    echo
    if [[ -n $version_current ]]; then
        print "* Version: $version_current ($git_hash)"
    fi
...
menu_usefulutilities() {
    ...
    while [[ -z "$mode" && -z "$back" ]]; do
        ...
        case $selected in
            "Hacktivate Device" )
                print "* Note: This is for hacktivating devices that are already restored and jailbroken with lik-mac-compat."
            ...
            "Revert Hacktivation" )
                ...
                print "* This option can only be used if the hacktivation is done using lik-mac-compat's \"Hacktivate Device\" option."
                select_yesno
                if [[ $? != 1 ]]; then
                    continue
                fi
            ...
            "Run uicache" )
                ...
                print "* If your device is restored/jailbroken with lik-mac-compat, OpenSSH is already installed."
                select_yesno
                if [[ $? != 1 ]]; then
                    continue
                fi
...
device_jailbreak_confirm() {
    ...
    if [[ $device_proc == 5 || $device_proc == 6 ]]; then
        ...
        if [[ $platform == "linux" ]]; then
            case $device_vers in
                [689]* | 10* ) print "* Note: If you need to sideload, you can use lik-mac-compat's \"Sideload IPA\" option.";;
            esac
        fi
    fi
    ...
    case $device_vers in
        ...
        [7654]* | 3.2* | 3.1.3 )
            if [[ $device_type == "iPhone2,1" && $device_vers == "3.1.3" ]]; then
                warn "To jailbreak iOS 3.x for the 3GS, make sure your device is restored with lik-mac-compat's \"Restore/Downgrade\" first."
...
restore_customipsw_confirm() {
    ...
    print "* This option is only for restoring with IPSWs NOT made with lik-mac-compat, like whited00r or GeekGrade."
    if [[ $device_newbr == 1 ]]; then
        ...
        warn "Do NOT use this option for powdersn0w or jailbreak IPSWs made with lik-mac-compat!"
    fi
...
main() {
    ...
    case $mode in
        ...
        "baseband" )
            ...
            print "* To stitch baseband to IPSW, run lik-mac-compat with --disable-bbupdate flag:"
        ...
        "actrec" )
            ...
            if (( device_proc < 7 )); then
                print "* To stitch activation to IPSW, run lik-mac-compat with --activation-records flag:"
...
if [[ $othertmp != 0 ]]; then
    ...
    print "* There might be other lik-mac-compat instance(s) running, or residual tmp folder(s) not deleted."
    print "* Running multiple instances is not fully supported and can cause unexpected behavior."
    print "* It is recommended to only use a single instance and/or delete all existing \"tmp\" folders in your lik-mac-compat folder before continuing."
